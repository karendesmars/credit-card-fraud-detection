...
- **Why does the same rebalancing technique work for one model and not the other?** SMOTE only changes the *training* set (creating synthetic fraud rows so the classes are 50/50); the test set keeps the real 0.17% fraud ratio and is never touched. Rebalancing the training data pushes any model to treat fraud as far more common than it really is - the two model types just react to that differently. LogisticRegression has a single global linear decision boundary, so this shift moves it aggressively toward flagging more transactions as fraud, which explains the recall jump but also the precision collapse. RandomForest, being an ensemble of trees, can carve out more localized decision regions instead of one global boundary, so it absorbs the rebalancing without losing much precision. This is confirmed by `class_weight="balanced"` (a rebalancing method that does not create synthetic rows) causing the exact same precision collapse for LogisticRegression (6.1% precision): so the issue is "rebalancing + linear model", not SMOTE specifically. SMOTE only pays off here when paired with RandomForest, which is in fact the best model in this comparison.
